...
public class Main {
    ...
    public static void addEntry(Scanner scanner, YearlyBudgets yearlyBudgets) {
        ...
        YearlyBudget yearCheck = new YearlyBudget(selectedYear);
        YearlyBudget selectedYearBudget = yearlyBudgets.checkYear(yearCheck);
    ...
    public static void viewEntries(Scanner scanner, YearlyBudgets yearlyBudgets) {
        ...
        Month monthBudget = yearlyBudgets.getYear(selectedYear).getMonth(selectedMonth);
    ...
    public static void viewNetBudget(Scanner scanner, YearlyBudgets yearlyBudgets) {
        ...
        System.out.println("Would you like to view net budget for a month or year? Type 1 for month or 2 for year ");
        ...
        if (selectedOption == 1) {
            ...
            netBudget = yearlyBudgets.getYear(selectedYear).getNetBudget();
            ...
            Month selectMonth = yearlyBudgets.getYear(selectedYear).getMonth(selectedMonth);
    ...
    public static void removeEntry(Scanner scanner, YearlyBudgets yearlyBudgets) {
        ...
        double selectedAmount = scanner.nextDouble();
        // creates an entry object with desired information
        Entry selectedEntry = new Entry(selectedName, selectedAmount);

        if (selectedOption == 1) {
            // loops through budget entries
            Budget budget = yearlyBudgets.getYear(selectedYear).getMonth(selectedMonth).getBudget();
            boolean budgetCheck = budget.checkEntry(selectedEntry);
            // if entry exists, remove the entry
            if (budgetCheck) {
                yearlyBudgets.getYear(selectedYear).getMonth(selectedMonth).getBudget().removeEntry(selectedEntry);
            ...
            Expenses expenses = yearlyBudgets.getYear(selectedYear).getMonth(selectedMonth).getExpenses();
            boolean expensesCheck = expenses.checkEntry(selectedEntry);
            if (expensesCheck) {
                yearlyBudgets.getYear(selectedYear).getMonth(selectedMonth).getExpenses().removeEntry(selectedEntry);
                System.out.println("The entry was removed! ");
            } else {
                // entry does not exist, error message and returns
                System.out.println("The entry does not exist! ");
